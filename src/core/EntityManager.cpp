...
EntityManager::Input( const Event& event )
{
    ...
    {
        if( ( event.type == ET_REPEAT ) && ( event.event == "walk_left" ) )
        {
            m_PlayerMove.x = -1;
        }
        else if( ( event.type == ET_REPEAT ) && ( event.event == "walk_right" ) )
        {
            m_PlayerMove.x = 1;
        }

        if( ( event.type == ET_REPEAT ) && ( event.event == "walk_down" ) )
        {
            m_PlayerMove.y = -1;
        }
        else if( ( event.type == ET_REPEAT ) && ( event.event == "walk_up" ) )
        {
            m_PlayerMove.y = 1;
        }

        if( ( event.type == ET_REPEAT ) && ( event.event == "run" ) )
        {
            m_PlayerRun = true;
        }

        if( ( event.type == ET_PRESS ) && ( event.event == "interact" ) )
        {
            CheckEntityInteract();
...
void
EntityManager::AddLineTrigger( const Ogre::String& name, const Ogre::Vector3& point1, const Ogre::Vector3& point2, const bool enabled )
{
    EntityTrigger* trigger = new EntityTrigger( name );
    trigger->SetLineTrigger( point1, point2 );
...
EntityManager::SetEntityOnWalkmesh( Entity* entity )
{
    ...
    for( unsigned int i = 0; i < m_Walkmesh.GetNumberOfTriangles(); ++i )
    {
...
*/
    ...
    // place entity on founded triangles
...
EntityManager::CheckTriggers( Entity* entity, Ogre::Vector3& position )
{
    ...
    {
        m_EntityTriggers[ i ]->CheckTrigger( entity, position );
    }
}
...
EntityManager::CheckEntityInteract()
{
    ...
    for( unsigned int i = 0; i < m_Entity.size(); ++i )
    {
        if( m_Entity[ i ]->IsInteractable() == false )
        {
            continue;
        }
        ...
        Ogre::Vector3 pos1 =  m_Entity[ i ]->GetPosition();
        float interact_range =  m_Entity[ i ]->GetInteractRadius();
