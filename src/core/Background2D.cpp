...
Background2D::SetScroll( const Ogre::Vector2& position )
{
    ...
    float half_width = 360.0f * aspect;
    float half_height = 360.0f;

    m_Position.x = ( m_Position.x + half_width > m_RangeMaxX ) ? m_RangeMaxX - half_width : m_Position.x;
    m_Position.x = ( m_Position.x - half_width < m_RangeMinX ) ? m_RangeMinX + half_width : m_Position.x;
    m_Position.y = ( m_Position.y + half_height > m_RangeMaxY ) ? m_RangeMaxY - half_height : m_Position.y;
    m_Position.y = ( m_Position.y - half_height < m_RangeMinY ) ? m_RangeMinY + half_height : m_Position.y;
