...
template<> AkaoParser* Ogre::Singleton< AkaoParser >::msSingleton = 0;
...
*/
        ...
        for( u32 bit = 1; channel_mask != 0; bit <<= 1, channel_id += 1 )
        {
            if ( channel_mask & bit )
            {
                ...
                offset_to_akao += 2;
                LOGGER->Log( "Init channel " + IntToString( channel_id ) + " with offset to AKAO " + HexToString( m_MusicChannelData[ channel_id ].akao_sequence_pointer, 4, '0' ) + "\n" );
...
void
AkaoParser::UpdateDynamicSpuParameters() // func2f848
{
    ...
    if( m_MusicChannelConfig.spu_update_flags & SPU_REVERB )
    {
        ...
        */
        ...
        UpdateSpuReverbDepth( 0x6, left, right );
    ...
    if( m_MusicChannelConfig.spu_update_flags & SPU_PITCH )
    {
        /*
        ...
        */
        ...
        m_MusicChannelConfig.spu_update_flags ^= SPU_PITCH;
    }
...
void
AkaoParser::UpdateSpuReverbDepth( u8 flag, u16 arg_left, u16 arg_right )
{
    if( flag < 1 || flag & 2 )
    {
        SPUwriteRegister( 0x1f801d84, arg_left ); // Reverberation depth left
        //[8004a6a8] = h(hu[A0 + 8]);
    }
    if( flag < 1 || flag & 4 )
    {
        SPUwriteRegister( 0x1f801d86, arg_right ); // Reverberation depth right
        //[8004a6aa] = h(hu[A0 + a]);
    }
}
...
AkaoParser::UpdateSequence( ChannelData* channel_data, int channel_id, ChannelConfig& channel_config )
{
    ...
    //LOGGER->Log( "UpdateSequence for channel " + IntToString( channel_id ) + "\n" );
    ...
    for( ;; )
    {
        ...
        else if( opcode == 0xa1 ) // load instrument
        {
            ...
            LOGGER->Log( "Load instrument " + IntToString( instrument_id ) + "with 0xA1.\n" );
...
*/
        ...
        else if( opcode == 0xa2 ) // pause
        {
            ...
            LOGGER->Log( "Set pause to 0x" + HexToString( value, 2, '0' ) + ".\n" );
            channel_data[ channel_id ].akao_sequence_pointer += 2;
        }
        else if( opcode == 0xa3 ) // set volume multiplier
        {
            ...
            log = "0xA3 Volume multiplier = 0x" + HexToString( channel_data[ channel_id ].volume_multiplier, 2, '0' ) + ".";
        }
        else if( opcode == 0xa5 ) // set pitch correction
        {
            ...
            LOGGER->Log( "Set pitch correction to 0x" + HexToString( channel_data[ channel_id ].pitch_correction, 4, '0' ) + ".\n" );
            channel_data[ channel_id ].akao_sequence_pointer += 2;
        }
        ...
        else if( opcode == 0xa8 ) // set volume
        {
            ...
            LOGGER->Log( "Set volume to 0x" + HexToString(channel_data[ channel_id ].volume_level, 8, '0' ) + " and reset volume_level_change_ticks.\n" );
            channel_data[ channel_id ].akao_sequence_pointer += 2;
        }
        else if( opcode == 0xa9 ) // set volume increase/descrease
        {
            ...
            LOGGER->Log( "Set volume tick to 0x" + HexToString( channel_data[ channel_id ].volume_level_change_ticks, 4, '0' ) + " and " );
            LOGGER->Log( "volume_level_add 0x" + HexToString( channel_data[ channel_id ].volume_level_add, 8, '0' ) + ".\n" );
        ...
        else if( opcode == 0xaa ) // set volume pan
        {
            ...
            LOGGER->Log( "Set volume pan to 0x" + HexToString( channel_data[ channel_id ].volume_index, 4, '0' ) + ".\n" );
            channel_data[ channel_id ].akao_sequence_pointer += 2;
        }
        else if( opcode == 0xad ) // ???
        {
        ...
        else if( opcode == 0xb4 ) // set wave 1
        {
            ...
            LOGGER->Log( "Set wave1 refresh interval to 0x" + HexToString( channel_data[ channel_id ].wave1_refresh_interval, 4, '0' ) + " " );
            LOGGER->Log( ", wave1 node index to 0x" + HexToString( channel_data[ channel_id ].wave1_table_node_index, 4, '0' ) + " " );
            LOGGER->Log( "and wave1 delay (or wave1 modifier) to 0x" + HexToString( value1, 2, '0' ) + ".\n" );
        ...
        else if( opcode == 0xb5 ) // set wave modifier
        {
            ...
            channel_data[ channel_id ].wave1_multiplier = ( ( ( channel_data[ channel_id ].wave1_modifier & 0x7f00) >> 8 ) * pitch_base ) >> 7;
            LOGGER->Log( "Set wave modifier to 0x" + HexToString( channel_data[ channel_id ].wave1_modifier, 4, '0' ) + ".\n" );
            channel_data[ channel_id ].akao_sequence_pointer += 2;
        }
        ...
        else if( opcode == 0xc8 ) // set loop point
        {
            ...
            LOGGER->Log( "Set loop index 0x" + HexToString( index, 4, '0' ) + ".\n" );
            channel_data[ channel_id ].akao_sequence_pointer += 1;
        }
        else if( opcode == 0xc9 ) // conditional jump
        {
            ...
            if( channel_data[ channel_id ].loop_count[ index ] != value )
            {
                ...
                LOGGER->Log( "Repeat conditional jump with index 0x" + HexToString( index, 4, '0' ) + " and value 0x" + HexToString( value, 4, '0' ) + ". Current loop count 0x" +  HexToString( channel_data[ channel_id ].loop_count[ index ], 4, '0' ) + "\n" );
            }
            else
            {
                channel_data[ channel_id ].loop_index = ( index - 1 ) & 3;
                LOGGER->Log( "Go through conditional jump with index 0x" + HexToString( index, 4, '0' ) + " and value 0x" + HexToString( value, 4, '0' ) + ".\n" );
            }
            channel_data[ channel_id ].akao_sequence_pointer += 1;
        }
        else if( opcode == 0xcc )
        ...
        else if( opcode == 0xe8 ) // set tempo
        {
            ...
            log = "0xE8 Tempo = 0x" + HexToString( channel_config.tempo, 8, '0' ) + ", tempo increment counter = 0.";
        }
        else if( opcode == 0xea ) // set reverb depth
        {
            ...
            log = "0xEA Reverb depth = 0x" + HexToString( channel_config.reverb_depth, 8, '0' ) + ", reverb depth increment counter = 0.";
        }
        else if( opcode == 0xec )
        {
        ...
        else if( opcode == 0xf2 ) // load instrument
        {
            LOGGER->Log( "    0xF2 " );
            int instrument_id = m_Music->GetU8( channel_data[ channel_id ].akao_sequence_pointer + 1 );
            LOGGER->Log( "Load instrument " + IntToString( instrument_id ) + " with 0xF2.\n" );
        ...
        else if( opcode == 0xfd ) // reset timers lower and upper
        {
            ...
            log = "0xFD Upper timer = 0 (bound " + HexToString( channel_config.upper_timer_top, 4, '0' ) + "), lower timer = 0 (bound " + HexToString( channel_config.lower_timer_top, 4, '0' ) + ").";
        }
        else if( opcode == 0xfe ) // reset timer top
        {
            ...
            log = "0xFE Top timer = " + HexToString( channel_config.top_timer, 4, '0' ) + ").";
        }
        else if( opcode == 0xff )
        {
        ...
        {
            LOGGER->Log( HexToString( opcode, 2, '0' ) + " [UNKNOWN]\n" );
            break;
        }

        LOGGER->Log( "    " + log + ".\n" );
        std::cout << channel_id  << ": " + log << std::endl;
    }
    ...
    if( opcode != 0xa0 )
    {
        LOGGER->Log( "    0x" + HexToString( opcode, 2, '0' ) + " PLAY.\n" );
