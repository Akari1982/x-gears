...
{
    Ogre::Camera* camera = Ogre::Root::getSingleton().getSceneManager( "Scene" )->getCamera( "Camera" );
    Ogre::Vector3 point2d = camera->getViewMatrix() * point;
    float z = point2d.z;
    point2d = camera->getProjectionMatrix() * point2d;
    point2d.z = z;

    float width = Ogre::Root::getSingleton().getRenderTarget( "QGearsWindow" )->getViewport( 0 )->getActualWidth() / 2;
    float height = Ogre::Root::getSingleton().getRenderTarget( "QGearsWindow" )->getViewport( 0 )->getActualHeight() / 2;

    point2d.x = width + point2d.x * width;
    point2d.y = height - point2d.y * height;

    if( point2d.z <= 0 )
    {
        float dist_sq = point.squaredDistance( camera->getPosition() );
...
DebugDraw::renderQueueEnded( Ogre::uint8 queueGroupId, const Ogre::String& invocation, bool& repeatThisInvocation )
{
    ...
    {
        m_RenderSystem->_setWorldMatrix( Ogre::Matrix4::IDENTITY );
        m_RenderSystem->_setViewMatrix( Ogre::Root::getSingleton().getSceneManager( "Scene" )->getCamera( "Camera" )->getViewMatrix() );
        m_RenderSystem->_setProjectionMatrix( Ogre::Root::getSingleton().getSceneManager( "Scene" )->getCamera( "Camera" )->getProjectionMatrix() );
